...
namespace ConceptGames.ConceptLineOrion.Guide
{
    ...
    public class ScoreReader : MonoBehaviour
    {
        ...
        [SerializeField] private float tapoffset;
        ...
        private void Update()
        {
            if (isplay)
            {
                if (index + 1 > hitTime.Count)
                    return;
                if (Player.Instance.gameTime > hitTime[index])
                {
                    index++;
                    AudioManager.PlayClip(turnClip);
                }
            }
        }
        private void Start()
        ...
        [Button("Create Tap Time", ButtonSizes.Large)]
        public void ReadTapTime()
        {
            box.Clear();
            hitTime.Clear();
            box.AddRange(boxHolder.GetComponentsInChildren<GuidelineTap>());
            for (int i = 0; i < box.Count; i++)
                hitTime.Add(box[i].triggerTime + tapoffset);
        }

        [Button("Create Guideline Taps By Score", ButtonSizes.Large)]
        private void CreateGuidelineTaps()
        {
            ...
            var hitParent = new GameObject("GuidelineTapHolder-ScoreCreated");
            var count = 1;
            ...
            for (var i = 0; i < hitTime.Count; i++)
            {
                ...
                focusedBox.transform.Translate(
                    i == 0
                        ? new Vector3(0, hitTime[i] * (float)speed, 0)
                        : new Vector3(0, (hitTime[i] - hitTime[i - 1]) * (float)speed, 0), Space.Self);
