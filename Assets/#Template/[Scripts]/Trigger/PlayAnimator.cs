...
namespace ConceptGames.ConceptLineOrion.Trigger
{
    ...
    public class SingleAnimator
    {
        ...
        {
            progress = animator.GetCurrentAnimatorStateInfo(0).normalizedTime % 1;
            playState = played;
        }
    }
    ...
    public class PlayAnimator : MonoBehaviour
    {
        ...
        {
            if (other.CompareTag("Player")) PlayAllAnimator();
        }

        public void PlayAllAnimator()
        {
            foreach (SingleAnimator a in animators) if (!a.played) a.PlayAnimator();
        }
    }
}